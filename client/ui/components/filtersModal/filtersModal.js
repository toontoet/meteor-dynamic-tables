...
function closeModal() {
  const filtersModalWrapper = $("#dynamic-table-filter-modal")[0];
  if (filtersModalWrapper) {
    Blaze.remove(filtersModalWrapper.__blazeTemplate);
    filtersModalWrapper.innerHTML = "";
  }
}
Template.dynamicTableFiltersModal.helpers({
...
Template.dynamicTableFiltersModal.events({
...
Template.dynamicTableFiltersModal.onRendered(function onRendered() {
  Tracker.autorun(() => {
    if (this.editing.get()) {
      Tracker.afterFlush(() => {
        if ($.fn.select2) {
...
Template.dynamicTableFiltersModal.onCreated(function onCreated() {
